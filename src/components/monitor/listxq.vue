  ...
  methods: {
    ...
    getItem({ url, sublayers, id, name, definitionExpression, ytd }, label) {
      ...
      loadModules(
        ...
        this.data = list;
        list.map(({ attributes }) => {
          const { XZQMC } = attributes;
          if (!XZQMC) return false;
          if (!this.sObj[XZQMC]) {
            this.sObj[XZQMC] = { XZQMC, count: 0 };
          }
          this.sObj[XZQMC].count++;
        });
        ...
        this.forceData = list.sort((a, b) => {
          const count1 = this.sObj[a.attributes.XZQMC].count;
          const count2 = this.sObj[b.attributes.XZQMC].count;

          if (count1 == count2) {
            return (
              this.countryHash[b.attributes.XZQMC] -
              this.countryHash[a.attributes.XZQMC]
            );
          }
