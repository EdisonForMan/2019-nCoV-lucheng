<template>
  <div id="btnDiv">
    <div class="btn-row">
      ...
      <button type="button" class="btnClass" :class="{active:dsjTag}" @click="dsj">大数据</button>
...
export default {
  ...
  methods: {
    ...
    // mapImageLayer 图层切换
    mapImageToggle(id) {
      this.$parent.$refs.cityArcgis.layerToggle(mapImageLayerList[id]);
    },

    cgdk() {
      this.$parent.$refs.cityArcgis.cgdk();
    },

    zddk() {
      this.$parent.$refs.cityArcgis.zddk();
    },
    xzqh() {
      this.$parent.$refs.cityArcgis.changeChanyePlate();
    },
    yxt() {
      this.$parent.$refs.cityArcgis.yxt();
    },
    slt() {
      this.$parent.$refs.cityArcgis.slt();
    },
    dsj() {
      this.$parent.$refs.cityArcgis.dsj();
    },
    kgt() {
    ...
    clean() {
      ...
      this.$parent.$refs.cityArcgis.view.graphics.removeAll();
      this.$parent.$refs.cityArcgis.view.popup.visible = false;
    },
    // 隐藏单个图层
    hideLayerById(id) {
      this.$parent.$refs.cityArcgis.map &&
        this.$parent.$refs.cityArcgis.map.findLayerById(id) &&
        (this.$parent.$refs.cityArcgis.map.findLayerById(id).visible = false);
    },
    // 隐藏多个图层
    hideLayerMany(arr) {
