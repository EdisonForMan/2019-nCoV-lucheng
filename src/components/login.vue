        ...
        auth_token({
          ...
          async msg => {
            ...
            if (that.show == 0) {
              ...
              window.location.href = `${WRT_config.lcSever}/wz-zddt/index.html?access_token=${msg}`;
            } else if (that.show == 1) {
              if (d.data.data[0].group.length) {
                ...
                list.map(({ au_groupname }) => {
                  if (~["管理组", "可运维管理"].indexOf(au_groupname)) {
                    window.location.href = `${WRT_config.lcManagerSever}/wz-zddt-manager/index.html#/index?access_token=${msg}`;
                  } else {
